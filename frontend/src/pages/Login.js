      ...
      <div style={s.card}>
        ...
        <form onSubmit={handleSubmit} style={s.form}>
          <div style={s.field}>
            ...
            <input name="email" type="email" value={form.email} onChange={handleChange} style={s.input} placeholder="you@example.com" required />
          </div>
          <div style={s.field}>
            <label style={s.label}>Password</label>
            <div style={{ position: 'relative' }}>
              <input name="password" type={showPass ? 'text' : 'password'} value={form.password} onChange={handleChange} style={{ ...s.input, paddingRight: '3rem' }} placeholder="Password" required />
              <button type="button" onClick={() => setShowPass(!showPass)} style={s.eyeBtn}>{showPass ? 'Hide' : 'Show'}</button>
            </div>
          </div>
        ...
        <div style={s.demo}>
          ...
          <div style={s.demoBtns}>
            <button onClick={() => setForm({ email: 'admin@rwandair.com', password: 'Admin@2024' })} style={s.demoBtn}>Admin</button>
            <button onClick={() => setForm({ email: 'agent@rwandair.com', password: 'Agent@2024' })} style={s.demoBtn}>Agent</button>
